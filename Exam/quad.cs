...
public static double adaptint(										// recursiv funktion, der kalder på sig selv igen og igen indtil den når limit/eller når et tilpas akkurat resultat. 
	...
	double acc=1e-6, double eps=1e-6, int limit=99,
	double f2=NaN		 											// genbruger punkt f2 og hvis ikke der er et punkt (dvs. NaN), så har vi defineret, hvordan de skal regnes ud. 
	){
	double f1=f(a+(b-a)/6), f3=f(a+5*(b-a)/6);						// End points by x_i = 1/6{1,3,5}   w_i = 1/8{3,2,3} (bereget) v_i = 1/3{1,1,1}
	...
	double err=Abs(Q-q)/Sqrt(2);									// rescaled absolute accuracy goal: d/sqrt(2)
		...
		double Q1=adaptint(f,a,				a+(b-a)/3,		acc/Sqrt(2),eps,limit-1,f1);// partition into three subintervals within interval [a,b], such that 
		double Q2=adaptint(f,a+(b-a)/3,		a+2*((b-a)/3),	acc/Sqrt(2),eps,limit-1,f2);// a < a+(b-a)/3 < a+2*(b-a)/3 <b
		double Q3=adaptint(f,a+2*((b-a)/3),	b,				acc/Sqrt(2),eps,limit-1,f3);
		return Q1+Q2+Q3;
		}
}//adapt4
