...
static void Main(){
	...
	int N=(int)10000;
	double[] r;
	r = montecarloint.miser(f,a,b,npoints:N,RND:new Random(5));
...
*/
	...
	r = montecarloint.plain(f,a,b,npoints:N,RND:new Random(5));
	Error.Write($"\n===== plain: x%x<R*R =====\n");
	Error.Write($"npoints={N}\n");
	Error.Write($"integ={r[0]:f5} ");
	Error.Write($"sigma={r[1]:f5}\n");
...
Error.Write($"error={Abs(exact-r[0]):f5} = {Abs(exact-r[0])/r[1]:f3} sigma\n");
	...
	r = montecarloint.miser(F,a,b,npoints:N);
...
Error.Write($"error={Abs(exact-r[0]):f5} = {Abs(exact-r[0])/r[1]:f3} sigma\n");
	...
	r = montecarloint.plain(F,a,b,npoints:N,RND:rnd);
