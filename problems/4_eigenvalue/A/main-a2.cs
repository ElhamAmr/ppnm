using System;
using static System.Console;
using static System.Math;
public class main{

public static void Main(string[] args){
int n=200;

var H = new matrix(n,n);
var e = new vector(n);
var V = new matrix(n,n);
double s=1.0/(n+1);
//MAtrix H:
for(int i=0;i<n-1;i++){
  matrix.set(H,i,i,-2);
  matrix.set(H,i,i+1,1);
  matrix.set(H,i+1,i,1);
  }
matrix.set(H,n-1,n-1,-2);
matrix.scale(H,-1/s/s);
int sweeps=jacobi.cyclic(H,e,V); //obs. ændrer i H. 

for (int k=0; k < Min(n/3,30); k++){
    double calculated = e[k];
    double exact = PI*PI*(k+1)*(k+1);
    WriteLine($"{k} {calculated} {exact}");
}

WriteLine();
WriteLine();

for(int k=0;k<5;k++){ //k: antallet af waves/bølger
  WriteLine($"{0} {0}");
  for(int i=0;i<n;i++){
	double factor=Sign(V[0,k])/Sqrt(s);
	WriteLine($"{(i+1.0)/(n+1)} {matrix.get(V,i,k)*factor}"); //kig lige i matlib efter get/set matrix
	}
  WriteLine($"{1} {0}\n");
  }


}//Main
}//main
